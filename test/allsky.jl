...
function vmr_2d_to_1d!(
    gas_conc::GasConcs{FT},
...
function all_sky(ds; use_luts = false, λ_string = "", compile_first = false)
  ...
  #
    ...
    t_lev,
    gas_conc_garand,
    col_dry = @timeit to "read_atmos" read_atmos(
        ds[:input],
        FT,
        I,
        gases_prescribed,
  ...
  # Clouds optical props are defined by band
    clouds_base = OpticalPropsBase(
        "Clouds",
        get_band_lims_wavenumber(k_dist.optical_props),
    )
  ...
  #   and not very close to the ground
    rel_val = FT(0.5) * (
        get_min_radius(cloud_optics_.liq) + get_max_radius(cloud_optics_.liq)
    )
    rei_val = FT(0.5) * (
        get_min_radius(cloud_optics_.ice) + get_max_radius(cloud_optics_.ice)
    )
    for ilay in 1:nlay
        for icol in 1:ncol
            cloud_mask[icol, ilay] = as.p_lay[icol, ilay] < FT(100) * FT(100) &&
                                     as.p_lay[icol, ilay] > FT(900)
      #
      ...
      #
            ...
            clouds_liq.re[icol, ilay] = fmerge(
                rel_val,
                FT(0),
                clouds_liq.wp[icol, ilay] > FT(0),
            )
            clouds_ice.re[icol, ilay] = fmerge(
                rei_val,
                FT(0),
                clouds_ice.wp[icol, ilay] > FT(0),
            )
        end
    end
  ...
  #
    ...
    for iloop in 1:(compile_first ? 1 : nloops)
    ...
    #
        ...
        if is_lw
            ...
            rte_lw!(fluxes, atmos, as.mesh_orientation, bcs, lw_sources)
        else
            ...
            rte_sw!(fluxes, atmos, as.mesh_orientation, bcs, μ_0)
        end
    ...
    if export_plots
        ...
        plot(
            heating_rate,
            z,
            title = "All sky $(λ_string) heating rates",
            xlabel = "heating rate",
            ylabel = "pressure",
        )
        out_dir = "output"
  ...
  # @show diff_dn, diff_dn_ulps, maximum(abs.(ref_flux_dn))
    if !compile_first
        if use_luts
            @test diff_up_ulps < sqrt(1 / (10 * eps(FT)))
            @test diff_dn_ulps < sqrt(1 / (10 * eps(FT)))
        else
      # Need better test
