...
function all_sky_pgp(ds; use_luts = false, λ_string = "")
    ...
    FT = Float64
    I = Int64
    p_lay, t_lay, p_lev, t_lev, gas_conc_garand, col_dry =
        @timeit to "read_atmos" read_atmos(ds[:input], FT, I, gases_prescribed)
    ...
    for ilay = 1:nlay
        for icol = 1:ncol
            cloud_mask[icol, ilay] =
                as.p_lay[icol, ilay] < FT(100) * FT(100) &&
