...
function draw_samples(
    cloud_mask::Array{Bool},
    clouds::T,
    clouds_sampled::T,
) where {T<:AbstractOpticalPropsArry}
  ...
  # Array extents
    ...
    @assert all([get_ncol(clouds_sampled), get_nlay(clouds_sampled)] .== [
        ncol,
        nlay,
    ])
  ...
  # Optical depth assignment works for 1scl, 2str (also nstr)
    apply_cloud_mask!(
        clouds_sampled.τ,
        get_band_lims_gpoint(clouds_sampled),
        cloud_mask,
        clouds.τ,
    )
...
function sampled_mask_max_ran!(randoms, cloud_frac, cloud_mask)
  ...
  # Construct the cloud mask for each column
    for icol in 1:ncol
        ...
        cloud_mask[icol, ilay, 1:ngpt] .= local_rands[1:ngpt] .> (
            FT(1) - cloud_frac[icol, ilay]
        )
        for ilay in cloud_lay_fst+1:cloud_lay_lst
    ...
    for icol in 1:ncol
        ...
        #
          ...
          #
                    ...
                    local_rands[1:ngpt] = rho * (
                        local_rands[1:ngpt] - FT(0.5)
                    ) +
                                          sqrt(FT(1) - rho * rho) * (
                        randoms[1:ngpt, ilay, icol] - FT(0.5)
                    ) +
                                          FT(0.5)
                else
                    local_rands[1:ngpt] = randoms[1:ngpt, ilay, icol]
...
function apply_cloud_mask!(
    ...
    for ibnd in 1:nbnd
        for igpt in band_lims_gpt[1, ibnd]:band_lims_gpt[2, ibnd]
            for ilay in 1:nlay
                sampled_field[1:ncol, ilay, igpt] .= merge.(
                    input_field[1:ncol, ilay, ibnd],
                    FT(0),
                    cloud_mask[1:ncol, ilay, igpt],
                )
            end
        end
    end
