...
@testset "Benchmark suite" begin
  ...
  ds_allsky_sw = dataset_dict(data_files_dict(datafolder, "sw"; allsky=true))
  ds_allsky_lw = dataset_dict(data_files_dict(datafolder, "lw"; allsky=true))
  ...
  !refresh_params && loadparams!(suite, BenchmarkTools.load(params_file)[1], :evals, :samples);
  !refresh_master && (master = BenchmarkTools.load(master_file)[1])

  suite["allsky"] = BenchmarkGroup(["clouds"])
  for use_luts in (true, false)
    suite["allsky"]["sw", string(use_luts)] =
      @benchmarkable all_sky($(ds_allsky_sw); use_luts=$(use_luts), λ_string=$("sw"))
  end

  for use_luts in (true, false)
    suite["allsky"]["lw", string(use_luts)] =
