...
function test_cos_zenith_edge_cases(
    ...
    # Test 1: No NaN values in shortwave fluxes (flux_up, flux_dn, flux_net)
    @test !any(isnan, flux_up_sw)
    @test !any(isnan, flux_dn_sw)
    @test !any(isnan, flux_net_sw)
    
    # Note: flux_dn_dir only has valid data at level 1 (surface) 
    # Levels 2-nlev may contain uninitialized values
    @test !any(isnan, flux_dn_dir_sw[1, :])

    # Test 2: No Inf values in shortwave fluxes
    @test !any(isinf, flux_up_sw)
    @test !any(isinf, flux_dn_sw)
    @test !any(isinf, flux_net_sw)
    @test !any(isinf, flux_dn_dir_sw[1, :])

    # Test 3: No NaN/Inf in longwave fluxes (longwave should be unaffected by cos_zenith)
    @test !any(isnan, flux_up_lw)
    @test !any(isnan, flux_dn_lw)
    @test !any(isnan, flux_net_lw)
    @test !any(isinf, flux_up_lw)
    @test !any(isinf, flux_dn_lw)
    @test !any(isinf, flux_net_lw)

    # Test 4: Column with normal cos_zenith (0.5) should have positive fluxes
    @test any(flux_up_sw[:, 1] .> FT(0))
    @test any(flux_dn_sw[:, 1] .> FT(0))
    @test flux_dn_dir_sw[1, 1] > FT(0)

    # Test 5: Columns with cos_zenith ≤ 0 should have zero shortwave fluxes
    ...
    # Test 6: Column with very small cos_zenith (1e-10) should still produce valid results
    ...
    # Test 7: Aerosol optical depth should be non-negative and physically consistent
    # Check no NaN or Inf in aerosol optical depths
    @test !any(isnan, as.aerosol_state.aod_sw_ext)
    @test !any(isnan, as.aerosol_state.aod_sw_sca)
    @test !any(isinf, as.aerosol_state.aod_sw_ext)
    @test !any(isinf, as.aerosol_state.aod_sw_sca)
    @test all(as.aerosol_state.aod_sw_ext .> FT(0))
    @test all(as.aerosol_state.aod_sw_sca .> FT(0))

    # Test 8: Verify all columns have the same AOD values (since we copied data from column 1 to all columns)
