...
function read_char_vec(ds, var_name)
  tmp = ds[var_name][:]
  return String[strip(join(tmp[:,i])) for i = 1:size(tmp, 2)]
end
...
function load_and_init(ds, ::Type{FT}, gases_prescribed::Vector{AbstractGas}) where {FT<:AbstractFloat}
  ...
  if haskey(ds,"totplnk")
    ...
    kdist = get_kdist_longwave(totplnk, planck_frac, rayl_lower, rayl_upper, ref, args...)
  else
    solar_src = ds["solar_source"][:]
    kdist = get_kdist_shortwave(solar_src, rayl_lower, rayl_upper, ref, args...)
  end
  return kdist
