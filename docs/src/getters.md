...
copy above). One subtlety: the solver's compute buffers are stored
column-first, `(ncol, nlev)`, for coalesced GPU access, and `update_fluxes!`
ends by transposing them into the `(nlev, ncol)` presentation arrays the flux
getters expose — so read fluxes after `update_fluxes!`, not between manual
Layer-1 solves.
