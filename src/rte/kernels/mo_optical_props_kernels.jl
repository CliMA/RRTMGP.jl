    ...
    for igpt = 1:ngpt
      for ilay = 1:nlay
        for icol = 1:ncol
          ...
          tau[icol,ilay,igpt] = (wp(1.0) - wf) * tau[icol,ilay,igpt]
          ssa[icol,ilay,igpt] = (ssa[icol,ilay,igpt] - wf) /  max(ε_machine(wp),(wp(1.0) - wf))
          g  [icol,ilay,igpt] = (g  [icol,ilay,igpt] - f[icol,ilay,igpt]) / 
                                        max(ε_machine(wp),(wp(1.0) - f[icol,ilay,igpt]))
        end
      end
    end
    ...
    for igpt = 1:ngpt
      for ilay = 1:nlay
        for icol = 1:ncol
          ...
          tau[icol,ilay,igpt] = (wp(1.0) - wf) * tau[icol,ilay,igpt]
          ssa[icol,ilay,igpt] = (ssa[icol,ilay,igpt] - wf) /  max(ε_machine(wp),(wp(1.0) - wf))
          g  [icol,ilay,igpt] = (g  [icol,ilay,igpt] -  f) /  max(ε_machine(wp),(wp(1.0) -  f))
        end
      end
    end
      ...
      for ilay = 1:nlay
        for icol = 1:ncol
          tau1[icol,ilay,igpt] = tau1[icol,ilay,igpt] + 
                                 tau2[icol,ilay,igpt] * ( wp(1.0) - ssa2[icol,ilay,igpt])
        end
      end
    end
  end
...
#    integer :: ibnd, igpt
    ...
    for ibnd = 1:nbnd
      for igpt = gpt_lims[1, ibnd]:gpt_lims[2, ibnd]
        tau1[:,:,igpt] = tau1[:,:,igpt] + tau2[:,:,ibnd] * ( wp(1.0) - ssa2[:,:,ibnd])
      end
    end
  end
  # ---------------------------------
...
#    integer :: icol, ilay, igpt
    ...
    for igpt = 1:ngpt
      for ilay = 1:nlay
        for icol = colS:colE
          tau_out[icol-colS+1, ilay, igpt] = tau_in[icol, ilay, igpt] * ( wp(1.0) - ssa_in[icol, ilay, igpt] )
        end
      end
    end
