...
struct RTEBase{FT<:AbstractFloat,I<:Int}
 ...
 # - `flux_dn` downward radiance, Top level must contain incident flux boundary condition
  ...
  function RTEBase(fluxes::FluxesBroadBand{FT},
                   mo::MeshOrientation{I},
                   bcs::AbstractRadiativeBoundaryConditions{FT},
                   optical_props::AbstractOpticalPropsArry{FT}) where {FT,I}
...
function RTELongWave(base::RTEBase{FT,I},
                     sources::SourceFuncLongWave{FT, I},
                     angle_disc::Union{GaussQuadrature{FT,I},Nothing},
                     optical_props::OneScalar{FT,I}) where {FT,I}
  ncol  = get_ncol(optical_props)
  nlay  = get_nlay(optical_props)
  ngpt  = get_ngpt(optical_props)
  ...
  sfc_emis_gpt = expand_and_transpose(optical_props, base.bcs.sfc_emis)
...
function RTELongWave(base::RTEBase{FT,I},
                     sources::SourceFuncLongWave{FT, I},
                     angle_disc::Union{GaussQuadrature{FT,I},Nothing},
                     optical_props::TwoStream{FT,I}) where {FT,I}
  ncol  = get_ncol(optical_props)
  nlay  = get_nlay(optical_props)
  ngpt  = get_ngpt(optical_props)
  @assert get_ncol(sources) == ncol
...
function RTEShortWave(base::RTEBase{FT,I},
                      μ_0::Array{FT},
                      optical_props::OneScalar{FT}) where {FT,I}
...
function RTEShortWave(base::RTEBase{FT,I},
                      μ_0::Array{FT},
                      optical_props::TwoStream{FT}) where {FT,I}
  # Lower boundary condition -- expand surface albedos by band to gpoints
  #   and switch dimension ordering
  sfc_alb_dir_gpt = expand_and_transpose(optical_props, base.bcs.sfc_alb_direct)
  sfc_alb_dif_gpt = expand_and_transpose(optical_props, base.bcs.sfc_alb_diffuse)
