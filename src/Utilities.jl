...
macro unpack_fields(stuff, syms...)
    ...
    for sym in syms
        push!(
            thunk.args,
            :($(esc(sym)) = getproperty($(esc(stuff)), $(QuoteNode(sym)))),
        )
    end
...
"""
check_range(val::Array, minV, maxV, label) =
    any(val .< minV) || any(val .> maxV) ?
    error(strip(label) * " values out of range.") : nothing
...
any_vals_outside(val, mask, checkMin, checkMax) =
    mask ? val < checkMin || val > checkMax : false

function reshape_for_comparison(
    flux::Array{FT},
    nlay::I,
    ncol::I,
    nexp::I,
