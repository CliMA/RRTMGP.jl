...
radiation_method(s::RRTMGPSolver) = s.radiation_method
parameters(s::RRTMGPSolver) = s.params
shortwave_solver(s::RRTMGPSolver) = s.sws
longwave_solver(s::RRTMGPSolver) = s.lws
atmospheric_state(s::RRTMGPSolver) = s.as
...
clear_lw_flux(s::RRTMGPSolver)                        = s.clear_flux_lw.flux
...
clear_sw_flux(s::RRTMGPSolver)                        = s.clear_sw_flux
cos_zenith(s::RRTMGPSolver)                           = s.sws.bcs.cos_zenith
weighted_irradiance(s::RRTMGPSolver)                  = s.sws.bcs.toa_flux
