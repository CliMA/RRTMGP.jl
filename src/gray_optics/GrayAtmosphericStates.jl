    ...
    for icol = 1:ncol
        if top_at_1
            ...
            for icls = 1:ncls
                ...
                z_lev[icls*poly_order+1, icol] = zend - icls * Δz_cell
            end
            ...
            p_lev[nlev, icol] = p0
            t_lev[nlev, icol] =
                tt * (1 + d0[icol] * (p_lev[nlev, icol] / p0)^α)^FT(0.25)

            for ilay = nlay:-1:1
                H =
                    FT(R_d(param_set)) * t_lev[ilay+1, icol] /
                    FT(grav(param_set))
                Δz_lay = abs(z_lev[ilay+1, icol] - z_lev[ilay, icol])
                p_lev[ilay, icol] = p_lev[ilay+1, icol] * exp(-Δz_lay / H)
                t_lev[ilay, icol] =
                    tt * (1 + d0[icol] * (p_lev[ilay, icol] / p0)^α)^FT(0.25)
                t_lay[ilay, icol] =
            ...
            p_lev[1, icol] = p0
            t_lev[1, icol] =
                tt * (1 + d0[icol] * (p_lev[icol, 1] / p0)^α)^FT(0.25)
...
@kernel function setup_gray_as_pr_grid_kernel!(
    ...
    if top_at_1
        ...
        p_lev[nlev, gcol] = p0
        t_lev[nlev, gcol] =
            tt * (FT(1) + d0[gcol] * (p_lev[nlev, gcol] / p0)^α)^FT(0.25)
        z_lev[nlev, gcol] = FT(0)

        for ilay = nlay:-1:1
            #                if step == "linear"
            p_lev[ilay, gcol] = p_lev[ilay+1, gcol] - Δp
            #                else
            #                    p_lev[ilay, gcol] = p_lev[ilay+1, gcol] * exp(-efac)
            #                end
            ...
            t_lev[ilay, gcol] =
                tt * (FT(1) + d0[gcol] * (p_lev[ilay, gcol] / p0)^α)^FT(0.25)
            t_lay[ilay, gcol] =
                tt * (FT(1) + d0[gcol] * (p_lay[ilay, gcol] / p0)^α)^FT(0.25)

            H = r_d * t_lay[ilay, gcol] / grav_
            Δz_lay = H * log(p_lev[ilay+1, gcol] / p_lev[ilay, gcol])
            z_lev[ilay, gcol] = Δz_lay + z_lev[ilay+1, gcol]
        end
    else
        #---bot_at_1------------------------------
        p_lev[1, gcol] = p0
        t_lev[1, gcol] =
            tt * (FT(1) + d0[gcol] * (p_lev[1, gcol] / p0)^α)^FT(0.25)
        z_lev[1, gcol] = FT(0)
    ...
    t_sfc[gcol] = (top_at_1 ? t_lev[end, gcol] : t_lev[1, gcol])
