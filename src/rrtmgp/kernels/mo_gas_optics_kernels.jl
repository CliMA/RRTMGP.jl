...
function interpolation(
  ...
  jtemp = Array{Int}(undef, ncol, nlay)
  fmajor = Array{FT}(undef, 2,2,2,nflav,ncol,nlay)
  fminor = Array{FT}(undef, 2,2,nflav,ncol,nlay)
  col_mix = Array{FT}(undef, 2, nflav, ncol, nlay)
  tropo = Array{Bool}(undef, ncol, nlay)
  jeta = Array{Int}(undef, 2, nflav, ncol, nlay)
  jpress = Array{Int}(undef, ncol, nlay)
...
function interpolation!(jtemp,fmajor,fminor,col_mix,tropo,jeta,jpress,
                        ncol,nlay,ngas,nflav,neta, npres, ntemp,
                        flavor,
                        press_ref_log, temp_ref::Array{FT},press_ref_log_delta,
                        temp_ref_min,temp_ref_delta,press_ref_trop_log,
                        vmr_ref,
                        play,
                        tlay,
    ...
    @inbounds for icol in 1:ncol
      ...
      @inbounds for iflav in 1:nflav
        ...
        @inbounds for itemp in 1:2
          ...
          @inbounds ratio_eta_half = vmr_ref[itropo,igases[1],(jtemp[icol,ilay]+itemp-1)] /
                           vmr_ref[itropo,igases[2],(jtemp[icol,ilay]+itemp-1)]
          @inbounds col_mix[itemp,iflav,icol,ilay] = col_gas[icol,ilay,igases[1]] + ratio_eta_half * col_gas[icol,ilay,igases[2]]
          @inbounds eta = fmerge(col_gas[icol,ilay,igases[1]] / col_mix[itemp,iflav,icol,ilay], FT(0.5),
                      col_mix[itemp,iflav,icol,ilay] > FT(2) * floatmin(FT))
          loceta = eta * FT(neta-1)
          @inbounds jeta[itemp,iflav,icol,ilay] = min(fint(loceta)+1, neta-1)
          feta = mod(loceta, 1)
...
"""
function compute_tau_absorption!(idx_h2o::I,
                                 gpoint_flavor::Array{I},
                                 band_lims_gpt::Array{I},
                                 kmajor::Array{FT},
                                 kminor_lower::Array{FT},
                                 kminor_upper::Array{FT},
                                 minor_limits_gpt_lower::Array{I},
                                 minor_limits_gpt_upper::Array{I},
                                 minor_scales_with_density_lower::Array{B},
                                 minor_scales_with_density_upper::Array{B},
                                 scale_by_complement_lower::Array{B},
                                 scale_by_complement_upper::Array{B},
                                 idx_minor_lower::Array{I},
                                 idx_minor_upper::Array{I},
                                 idx_minor_scaling_lower::Array{I},
                                 idx_minor_scaling_upper::Array{I},
                                 kminor_start_lower::Array{I},
                                 kminor_start_upper::Array{I},
                                 tropo::Array{B},
                                 col_mix::Array{FT},
                                 fmajor::Array{FT},
                                 fminor::Array{FT},
                                 play::Array{FT},
                                 tlay::Array{FT},
                                 col_gas::AbstractArray{FT}, # OffsetArray
                                 jeta::Array{I},
                                 jtemp::Array{I},
                                 jpress::Array{I}) where {FT<:AbstractFloat, I<:Integer, B<:Bool}

  ncol,nlay = size(play)
  s_kmajor = size(kmajor)
  ngpt,neta,npres,ntemp = s_kmajor[1], s_kmajor[2], s_kmajor[3]-1,s_kmajor[4]
  nminorklower = size(kminor_lower,1)
  nminorkupper = size(kminor_upper,1)
  nminorlower = length(scale_by_complement_lower)
  nminorupper = length(scale_by_complement_upper)
  nbnd = size(band_lims_gpt,2)
  ngas = size(col_gas,3)-1
  nflav = size(fminor,3)
  ...
  if top_at_1
    @inbounds itropo_lower[:, 1] .= fminloc_wrapper(play, dim=2, mask=tropo)
    @inbounds itropo_lower[:, 2] .= nlay
    @inbounds itropo_upper[:, 1] .= 1
    @inbounds itropo_upper[:, 2] .= fmaxloc_wrapper(play, dim=2, mask=(.!tropo)) # TODO:
  else
    itropo_lower[:, 1] .= 1
  ...
  # Major Species
  # ---------------------
  gas_optical_depths_major!(gpoint_flavor,
        band_lims_gpt,
        kmajor,
        col_mix,fmajor,
  ...
  # Minor Species - lower
  # ---------------------
  gas_optical_depths_minor!(idx_h2o,
         gpoint_flavor[1,:],
         kminor_lower,
...
integer :: gptS, gptE
"""
function gas_optical_depths_major!(gpoint_flavor::Array{I},
                                   band_lims_gpt::Array{I},    # inputs from object
                                   kmajor::Array{FT},
                                   col_mix,
                                   fmajor::Array{FT},
                                   jeta::Array{I},
                                   ...
                                   jtemp::Array{I},
                                   jpress::Array{I},
                                   tau::Array{FT}) where {FT<:AbstractFloat, I<:Integer, B<:Bool}
  ncol,nlay = size(tropo)
  nbnd = size(band_lims_gpt,2)
  nflav = size(col_mix,2)
  s_kmajor = size(kmajor)
  ngpt,neta,npres,ntemp = s_kmajor[1], s_kmajor[2], s_kmajor[3]-1,s_kmajor[4]
  ...
  @inbounds for icol in 1:ncol
    @inbounds for ilay in 1:nlay
      ...
      @inbounds for ibnd in 1:nbnd
        @inbounds gptS = band_lims_gpt[1, ibnd]
        @inbounds gptE = band_lims_gpt[2, ibnd]
        @inbounds iflav = gpoint_flavor[itropo, gptS] #eta interpolation depends on band's flavor
        @inbounds tau_major[gptS:gptE] .=
...
"""
function gas_optical_depths_minor!(idx_h2o::I,
                                   gpt_flv::Array{I},
                                   kminor::Array{FT},
                                   minor_limits_gpt::Array{I},
                                   minor_scales_with_density::Array{B},
                                   scale_by_complement::Array{B},
                                   idx_minor::Array{I},
                                   idx_minor_scaling::Array{I},
                                   kminor_start::Array{I},
                                   play::Array{FT},
                                   tlay::Array{FT},
                                   col_gas::AbstractArray{FT}, # OffsetArray
                                   fminor::Array{FT},
                                   jeta::Array{I},
                                   layer_limits::Array{I},
                                   jtemp::Array{I},
                                   tau::Array{FT}) where {FT<:AbstractFloat,I<:Integer,B<:Bool}
  nminork,neta,ntemp = size(kminor)
  nminor = length(kminor_start)
  ngpt = length(gpt_flv)
  ncol,nlay = size(play)
  nflav = size(fminor,2)
  ngas = size(col_gas,3)-1
      ...
      @inbounds for icol in 1:ncol
        ...
        if layer_limits[icol,1] > 0
          @inbounds for ilay in layer_limits[icol,1]:layer_limits[icol,2]
            ...
            #
            @inbounds scaling = col_gas[icol,ilay,idx_minor[imnr]]
            #
            ...
            if minor_scales_with_density[imnr]
              ...
              #
              @inbounds scaling = scaling * (PaTohPa(FT)*play[icol,ilay]/tlay[icol,ilay])
              if idx_minor_scaling[imnr] > 0  # there is a second gas that affects this gas's absorption
                ...
                if scale_by_complement[imnr] # scale by densities of all gases but the special one
                  @inbounds scaling = scaling * (FT(1) - col_gas[icol,ilay,idx_minor_scaling[imnr]] * vmr_fact * dry_fact)
                else
                  @inbounds scaling = scaling *          col_gas[icol,ilay,idx_minor_scaling[imnr]] * vmr_fact * dry_fact
                end
              end
            end
            #
            ...
            @inbounds gptS = minor_limits_gpt[1,imnr]
            @inbounds gptE = minor_limits_gpt[2,imnr]
            @inbounds iflav = gpt_flv[gptS]
            @inbounds tau_minor[gptS:gptE] = scaling *
            ...
            @inbounds tau[gptS:gptE,ilay,icol] = tau[gptS:gptE,ilay,icol] + tau_minor[gptS:gptE]
          end
        end
      end
    end
...
# -----------------
"""
function compute_tau_rayleigh!(gpoint_flavor::Array{I},
                               band_lims_gpt::Array{I},
                               krayl::Array{FT},
                               idx_h2o::I,
                               col_dry::Array{FT},
  ...
  @inbounds for ilay in 1:nlay
    @inbounds for icol in 1:ncol
      @inbounds itropo = fmerge(1,2,tropo[icol,ilay]) # itropo = 1 lower atmosphere; itropo = 2 upper atmosphere
      @inbounds for ibnd in 1:nbnd
        @inbounds gptS = band_lims_gpt[1, ibnd]
        @inbounds gptE = band_lims_gpt[2, ibnd]
        @inbounds iflav = gpoint_flavor[itropo, gptS] #eta interpolation depends on band's flavor
        @inbounds interpolate2D_byflav!(@view(k[gptS:gptE]), fminor[:,:,iflav,icol,ilay],
                                            krayl[:,:,:,itropo],
                                            gptS, gptE, jeta[:,iflav,icol,ilay], jtemp[icol,ilay])
        @inbounds tau_rayleigh[gptS:gptE,ilay,icol] = k[gptS:gptE] *
...
integer :: igpt
"""
function interpolate2D_byflav(fminor::Array{FT},
                              k::Array{FT},
                              gptS::I, gptE::I,
                              jeta::Array{I}, jtemp::I) where {FT<:AbstractFloat, I<:Integer}
  res = Vector{FT}(undef, gptE-gptS+1)
  interpolate2D_byflav!(res, fminor, k, gptS, gptE, jeta, jtemp)
  return res
end
function interpolate2D_byflav!(res::AbstractArray{FT},
                               fminor::Array{FT},
                               k::Array{FT},
                               gptS::I, gptE::I,
                               jeta::Array{I}, jtemp::I) where {FT<:AbstractFloat, I<:Integer}
  # each code block is for a different reference temperature
  @inbounds for igpt in eachindex(res)
...
function interpolate3D_byflav(scaling, fmajor, k, gptS, gptE, jeta, jtemp, jpress)
  ...
  FT = eltype(k)
  res = Vector{FT}(undef, gptE-gptS+1)
  interpolate3D_byflav!(res, scaling, fmajor, k, gptS, gptE, jeta, jtemp, jpress)
  return res
end
function interpolate3D_byflav!(res::Array{FT}, scaling, fmajor, k, gptS, gptE, jeta, jtemp, jpress) where FT
  # each code block is for a different reference temperature
