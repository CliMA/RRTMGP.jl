...
"""
function extrap_lower(
    p_lay::Array{FT},
    p_lev::FT,
    ϕ_lay::Array{FT},
) where {FT<:AbstractFloat}
    return ϕ_lay[1] +
           (p_lev - p_lay[1]) * (ϕ_lay[2] - ϕ_lay[1]) / (p_lay[2] - p_lay[1])
end
...
"""
function extrap_upper(
    p_lay::Array{FT},
    p_lev::FT,
    ϕ_lay::Array{FT},
) where {FT<:AbstractFloat,I<:Int}
    return ϕ_lay[2] +
           (p_lev - p_lay[2]) * (ϕ_lay[2] - ϕ_lay[1]) / (p_lay[2] - p_lay[1])
end
...
"""
function interpolate_var(
    p_lay::Array{FT},
    p_lev::FT,
    ϕ_lay::Array{FT},
) where {FT<:AbstractFloat,I<:Int}
    return (
        p_lay[1] * ϕ_lay[1] * (p_lev - p_lay[2]) +
...
function interpolate_var(
  ...
  #   Interpolation and extrapolation at boundaries is weighted by pressure
    ...
    for ilay in 2:nlay
        for icol in 1:ncol
            ϕ_lev[icol, ilay] = interpolate_var(
                p_lay[icol, ilay-1:ilay],
                p_lev[icol, ilay],
                ϕ_lay[icol, ilay-1:ilay],
            )
        end
    end
    for icol in 1:ncol
        ϕ_lev[icol, nlay+1] = extrap_upper(
            p_lay[icol, nlay-1:nlay],
            p_lev[icol, nlay+1],
            ϕ_lay[icol, nlay-1:nlay],
        )
    end
    return ϕ_lev
end
