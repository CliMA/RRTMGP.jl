...
#
# -------------------------------------------------------------------------------------------------
function extract_subset_dim1_3d(
    ncol,
    nlay,
    ngpt,
    array_in,
...
# ---------------------------------
function extract_subset_dim2_4d(
    nmom,
    ncol,
    nlay,
    ngpt,
    ...
    for igpt = 1:ngpt
        for ilay = 1:nlay
            for icol = colS:colE
                for imom = 1:nmom
                    array_out[imom, icol-colS+1, ilay, igpt] =
                        array_in[imom, icol, ilay, igpt]
                end
            end
        end
    end
...
#    integer :: icol, ilay, igpt
function extract_subset_absorption_tau(
    ncol,
    nlay,
    ngpt,
    tau_in,
...
function get_subset_range(
    ...
    @assert !(start < 1)
    @assert !(
        this.ncol ≠ nothing && start > this.ncol || start + n - 1 > this.ncol
    )
    ...
    for i = 1:size(this.gas_name)
        ...
        if size(this.concs[i].conc, 1) > 1      # Concentration stored as 2D
            subset.concs[i].conc[:, :] .=
                this.concs[i].conc[start:(start+n-1), :]
        else
